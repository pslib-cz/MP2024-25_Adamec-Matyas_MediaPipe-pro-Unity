...
public class SandBoxHandManager : MonoBehaviour
{
    public static SandBoxHandManager Instance;
    ...
    HandExtension _handExtension;

    private void Awake()
    {
        Instance = this;
        realSenseManager = RealSenseManagerFactory.GetManager();
        _handExtension = new HandExtension();
    }
    ...
    private Vector3 AdjustLocation(NormalizedLandmark normalizedLandmark, (float x, float y, float z) wristLandMark, (float x, float y, float z) realsensePosition)
    {
        Vector3 position = new Vector3(
            ...
            ((1 - (normalizedLandmark.z - wristLandMark.z)) * scaleFactor) - scaleFactor - realsensePosition.z * movementFactor + zOffSet
            );
        return position;
    }
    ...
    float lastz = 0;
    private void AdjustLandmarkPosition(List<NormalizedLandmark> landmarks, GameObject[] landmarkObjects, bool isRight, GameObject single)
    {
        ...
        realsensePosition = getPositionFromDepthTexture(wristLandMark);
        ...
        for (int i = 0; i < landmarkObjects.Length; i++)
        {
            ...
            // adjust single landmark
            single.transform.position = new Vector3( position.x, 0f, position.z );
    ...
    private void Update()
    {
        ...
        else
        {
            ...
            AdjustLandmarkPosition(_landmarksL, landmarksL, false, SingleLandMarkL);
            //_handExtension.UpdateLandmarks(landmarksL);
        }
        ...
        else
        {
            ...
            AdjustLandmarkPosition(_landmarksR, landmarksR, true, SingleLandMarkR);
            _handExtension.UpdateLandmarks(landmarksR);
        }
