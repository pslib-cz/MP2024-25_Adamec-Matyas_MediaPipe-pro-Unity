...
namespace Mediapipe.Unity
{
  public class StreamingAssetsResourceManager : IResourceManager
  {
    ...
    IEnumerator IResourceManager.PrepareAssetAsync(string name, string uniqueKey, bool overwriteDestination)
    {
      ...
      {
        UnityEngine.Debug.Log($"{_TAG} - {name} will not be copied to {destFilePath} because it already exists");
        yield break;
      }
      ...
      UnityEngine.Debug.Log($"{_TAG} ,Copying {sourceFilePath} to {destFilePath}...");
      File.Copy(sourceFilePath, destFilePath, overwriteDestination);
      UnityEngine.Debug.Log($"{_TAG} -  {sourceFilePath} is copied to {destFilePath}");
    }
    ...
    private IEnumerator CreateCacheFile(string assetName)
    {
      ...
      using (var webRequest = UnityWebRequest.Get(assetPath))
      {
        ...
        if (webRequest.result == UnityWebRequest.Result.Success)
        {
          ...
          UnityEngine.Debug.Log($"{_TAG} - Writing {assetName} data to {cacheFilePath}...");
          var bytes = webRequest.downloadHandler.data;
          File.WriteAllBytes(cacheFilePath, bytes);
          UnityEngine.Debug.Log($"{_TAG} - {assetName} is saved to {cacheFilePath} (length={bytes.Length})");
        }
        else
        {
          throw new System.Exception($"Failed to load {assetName}: {webRequest.error}");
        }
      }
#endif
    }
